...
void Checker::check()
{
    ...
    while (true) {
         ...
         * Reading Auth.xml
         */
        const tuple<string, bool> &auth = getData("http://192.168.0.5/chk.cgi?userid=0&userpwd=4711");
        if (get<1>(auth) == true) {
         ...
         * Reading Data.xml
         */
        string nowdate = dateToNum(boost::posix_time::second_clock::local_time());
        const tuple<string, bool> &data = getData("http://192.168.0.5/query.cgi?userid=0&sdate=" + nowdate + "&edate=" + nowdate + "&start=0&pagesize=3&user=" + userName + "&userkey=" + userKey);
        if (get<1>(data) == true) {
