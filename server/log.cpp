...
string Log::makeKeyMsg(const string &user, const string &key) const
{
    string out;
...
void Log::keys(const string &user, const string &key)
{
    const string& msg = makeKeyMsg(user, key);
    cout << msg << endl;
    ...
    try {
        ofstream log;
        log.open(_keyPath, ios::out|ios::ate|ios::app);
        log << msg << "\n";
        log.close();
    }
    catch (...) {
